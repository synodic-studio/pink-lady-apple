...
1. **Run `synodic-apple:testflight-ship` step 4** — first build per app needs 4c (create internal group) + 4d (add tester). Subsequent builds just need 4f (verify with `group-builds`, `list-testers`). The `list-groups --app-id <id>` check is the canary: if only an external group exists (`internal=False`), create the internal group.
...
claude plugins update synodic-apple@pink-lady
```

If that STILL doesn't pick up the new version, force-reinstall:
```bash
claude plugins uninstall synodic-apple@pink-lady
claude plugins install synodic-apple@pink-lady
```
